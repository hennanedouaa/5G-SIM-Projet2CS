...
import docker
# made by hennane douaa el ikhlas and Maha remil
# Configure logging
logging.basicConfig(
    level=logging.INFO,
...
def prompt_for_coordinates():
    """Prompt user for coordinates and return them"""
    # Get UERANSIM coordinates from docker-compose file
    try:
        ueransim_lat, ueransim_lon = get_ueransim_coords_from_docker_compose()
    except Exception as e:
        logging.error(f"Error getting UERANSIM coordinates: {str(e)}")
        ueransim_lat, ueransim_lon = 0.0, 0.0
    ...
    return {
        'ueransim': (ueransim_lat, ueransim_lon),
        'i_upf': (i_lat, i_lon),
        'psa_upf': (p_lat, p_lon)
    }

def apply_distance(coordinates):
    """Apply bandwidth limitations based on coordinates"""
    try:
        # Unpack coordinates
        ueransim_lat, ueransim_lon = coordinates['ueransim']
        i_upf_lat, i_upf_lon = coordinates['i_upf']
        psa_upf_lat, psa_upf_lon = coordinates['psa_upf']
